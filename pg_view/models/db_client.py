    ...
    def detect_db_connection_arguments(self):
        ...
        if not conn_args:
            ...
            if not conn_args:
                ...
                loggers.logger.error(msg.format(self.work_directory))
                return None
        ...
        if not result:
            loggers.logger.error('unable to connect to PostgreSQL cluster at {0} using any of the detected connection '
...
class DBClient(object):
    ...
    def establish_user_defined_connection(self, instance, clusters):
        ...
        try:
            ...
            loggers.logger.error('failed to establish connection to {0} via {1}'.format(instance, conn_params))
        ...
        if pid is None:
            loggers.logger.error('failed to read pid of the postmaster on {0}'.format(conn_params))
        ...
        if pid in pids:
            ...
            loggers.logger.error('duplicate connection options detected  for databases {0} and {1}, '
